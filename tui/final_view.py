...
class FinalView:
    ...
    def show(self):
        ...
        while True:
            ...
            self.stdscr.addstr(sheet_y, sheet_x, spent_str, (curses.color_pair(utils.COLOR_GREEN) if remaining == 0 else curses.color_pair(utils.COLOR_YELLOW)) | curses.A_BOLD); sheet_y += 2
            ...
            for cat in ["disciplines", "backgrounds"]:
                if getattr(self.character, cat) and y_right < max_draw_y:
                    self.stdscr.addstr(y_right, col2_x, f"═══ {cat.upper()} ═══", curses.color_pair(utils.COLOR_CYAN) | curses.A_BOLD); y_right += 1
            ...
            self.stdscr.addstr(start_y + container_height - 2, start_x + (container_width - len(exit_msg)) // 2, exit_msg, curses.color_pair(utils.COLOR_GREEN) | curses.A_BOLD)
            self.stdscr.refresh()
